...
public class CertainBookStore implements BookStore, StockManager {
    ...
    public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
        ...
        // Use a minimum priority queue to avoid sorting the values afterwards
        PriorityQueue<BookStoreBook> ratedBooks = new PriorityQueue<BookStoreBook>(numBooks + 1,
                new Comparator<BookStoreBook>() {
                    @Override
                    public int compare(BookStoreBook bookStoreBook, BookStoreBook bookStoreBook2) {
                        return Float.compare(bookStoreBook.getAverageRating(), bookStoreBook2.getAverageRating());
                    }
                });
        ...
        for (BookStoreBook book : bookMap.values()) {
            ratedBooks.add(book);
            if (ratedBooks.size() > numBooks) {
                ratedBooks.poll();
            }
        }

        // Make the top books immutable and return them
        List<Book> topRatedBooks = new ArrayList<Book>(ratedBooks);
